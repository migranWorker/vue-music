    ...
    <div class="songsheet">
        <div class="back" v-show="show">
            <div class="back-box" >
                ...
                {{singerObj.artist.name}}
            </div>
        </div>
        <div class="bscroll" ref="bscroll">
        <div class="bscroll-container">
        <div class="header" :style="{backgroundImage: 'url(' + singerObj.artist.img1v1Url + ')'}">
            <div class="header-box">
                <div class="header-title" @click="back">
                     返回
                </div>
                <div class="header-bottom">
                    <p>{{singerObj.artist.name}}</p>
                </div>
            </div>
        </div>
        <div class="song-sum">
        ...
        <div class="list" 
            v-for="(item,index) in singerObj.hotSongs"
            :key='item.id'
        >
            <span>{{index + 1}}</span>
...
export default {
  ...
  created() {
    ...
    this.$http.get(`/artists?id=${query.id}`).then(res => {
